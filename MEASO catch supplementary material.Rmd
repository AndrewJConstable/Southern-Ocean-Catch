...
fnPropInMareaForRule<-function(cbed_a            # data frame - with Grouping variables,"Depth","Area".
  ...
  Groups<-dimnames(cbed_a)[[2]] 
  Groups<-Groups[which(Groups %in% c("MEASO","ASD"))]

  if(nrow(cbed_a)>0) cbed_a[,"Area"] <- cbed_a[,"Area"]/sum(cbed_a[,"Area"]) else
                     cbed_a<-NULL
...
cat(cat("Total catch in CSB for ("),cat(CatchTaxa,sep=", "),cat(": ", sum(CSBdata[which(CSBdata[,"Catch_TXN_Code"] %in% Taxa),"Green_Weight"])),"\n",sep="")
