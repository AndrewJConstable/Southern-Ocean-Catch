...
if(plotToothfish){
  Depth_intervals[,"Minimum"]<-rep(-550,nrow(Depth_intervals))
  Depth_intervals[,"Maximum"]<-rep(-1800,nrow(Depth_intervals))
} # end if plot
...
fnPropInMareaForRule<-function(cbed_a            # data frame - with Grouping variables,"Depth","Area".
  ...
  if(length(Groups)==2){
    ...
    if(Groups[1]=="ASD"){
          res<-as.data.frame(matrix(0,ncol=1,nrow=length(ASD),dimnames=list(ASD,"Prop")))
        if(!is.null(cbed_a)) {
          res[match(cbed_a[,"ASD"],ASD),1]<-round(cbed_a[,"Area"],3)
        } # end if
...
fnCatchDistRule<-function(Rname,Rsubset,Rdescription,typeDI,DI,CBa,CbedSubset,Agroups,MEASO,ASD,DepthAdjust){
  
   dfAreas<-fnDepthArea(typeDI,DI,CBa[CbedSubset,c(Agroups,"Depth","Area")])
   
  if(is.null(dfAreas)) {

            CDR<- list(list( Rule    = Rname
                        ,Subset      = Rsubset
...
RuleDescription <- "Subarea 48.2- groundfish"
...
RuleName        <- "58B43_S"
TypeDepthInterval<-1 # row of data frame Depth_intervals
RuleDescription <- "Division 58.4.3 - AUS trawl"
...
CbedAreaSubset <-  
          (((  Cbed_rules[,"ASD"]=="58.4.3a" | Cbed_rules[,"ASD"]=="58.4.3b"
           ) & Cbed_rules[,"MEASO"]=="CIS")
                   & (   Cbed_rules[,"Depth"] <  Depth_intervals[TypeDepthInterval,"Minimum"]
                       & Cbed_rules[,"Depth"] >= Depth_intervals[TypeDepthInterval,"Maximum"])
                   ) # end subset
...
RuleName        <- "58M43_A"
TypeDepthInterval<-6 # row of data frame Depth_intervals
RuleDescription <- "Division 58.4.3 - krill - Central Indian Antarctic area"
RuleSubset <- (  CSBdata[,"GAR_Code"] == "5843"  
               & CSBdata[,"GTY_rev"]  == "OTM"
               & CSBdata[,"Flag_CTY_Code"] == "JPN"
    #          & CSBdata[,"TFN"] 
...
# lapply(CatchDistRules,function(cd){print(cd$Distribution)})
...
##############################################################################################
RuleName        <- "88M3_A"
TypeDepthInterval<-9 # row of data frame Depth_intervals
RuleDescription <- "Subarea 88.3 - all fishing - East Pacific Antarctic area"
RuleSubset <- (  CSBdata[,"GAR_Code"] == "883"  
               & CSBdata[,"GTY_rev"]  == "OTM"      
    #          & CSBdata[,"Flag_CTY_Code"]
...
for(c in seq(1,length(CatchDistRules),1)){ # too memory intensive to do lapply
   if(CatchDistRules[[c]]$Area_km2!=0){
        ...
        Pixels[CatchDistRules[[c]]$CbedAreaSubset] <- 
                            Pixels[CatchDistRules[[c]]$CbedAreaSubset] + 
                            Density * 
                            Cbed_rules[CatchDistRules[[c]]$CbedAreaSubset,"Area"]  
   } else {
     RuleZeroArea<-c(RuleZeroArea,c,CatchByRules[c])
   }
        } # end loop c

cat(cat("Total catch in CSB for ("),cat(CatchTaxa,sep=", "),cat(": ", sum(CSBdata[which(CSBdata[,"Catch_TXN_Code"] %in% CatchTaxa),"Green_Weight"])),"\n",sep="")

cat("Catch allocated to pixels: ",sum(Pixels),"\n",sep="")
...
library(measoshapes) # from GitHub 
library(dplyr)
library(sf)
library(RColorBrewer)

shapeCRS<-"+proj=longlat +datum=WGS84 +no_defs"
shapeCoast <- rgdal::readOGR(dsn="C:/_w/maps/shapefiles/Antarctic coastline", layer="Aad_coastline Drawing",p4s=shapeCRS)
shapeCoast <- sp::spTransform(shapeCoast, CRS(prj))

shapeIslands <- rgdal::readOGR(dsn="C:/_w/maps/shapefiles/subantarctic_islands", layer="gshhs_f_30-60s")
shapeIslands <- sp::spTransform(shapeIslands, CRS(prj))


measo <- measo_regions05 %>% group_by(name) %>% summarize() %>% 
  inner_join(measo_names)
# remove temperate areas
measo<-measo[-which(tmp1$name %in% c("AOT","CIT","EIT","WPT","EPT")),]


rgg<-as.data.frame(r_small,xy=2)

# convert rgg$layer to colour
col_pal<-colorRampPalette(c("yellow","green","seagreen","navyblue")) # from brewer
...
p<-ggplot(measo) 
p<-p + geom_sf(colour = "red", fill=NA, size=0.5) #, aes(fill=NULL)
p<-p+scale_fill_identity()
#p<-p+  theme(
#    panel.background = element_rect(fill = "transparent"), # bg of the panel
#    plot.background = element_rect(fill = "transparent", color = NA), # bg of the plot
#  )
p<-p+labs(x = NULL, y =NULL,title="Toothfish catch density (tonnes per square km)")
p<-p+geom_raster(data=rgg,mapping=aes(x,y,fill=rColour),na.rm=TRUE)
p<-p+ geom_sf(data = shapeCoast) # , aes(fill = NULL)
#p<-p+ geom_sf(data = shapeIslands)#, aes(fill = NULL)
p

library(SOmap)
ticks<-seq(-6,0,1)
SOmap(graticules=TRUE,graticules_col="grey50",trim=-40)
plot(measo,col=NA,border="red",add=TRUE)
...
SOleg(r_small, position = "topright", col = rCols
      ...
      trim = -35, label = "log10 (t/km2)", type = "continuous")
...
shapeIslands <- rgdal::readOGR(dsn="C:/_w/maps/shapefiles/subantarctic_islands", layer="gshhs_f_30-60s")
shapeIslands <- sp::spTransform(shapeIslands, CRS(prj))
shapeMEASO <- rgdal::readOGR(dsn="C:/_w/maps/shapefiles/Measo_areas_shapefiles",layer="Measo_areas_shapefiles",p4s=shapeCRS)
shapeMEASO <- sp::spTransform(shapeMEASO, CRS(prj))

plot(r )
plot(shapeIslands,col="black", lwd=0.5, add=TRUE)
plot(shapeCoast,col="black", lwd=0.5, add=TRUE)
plot(shapeMEASO,border="red", add=TRUE)


plot(shapeCoast,col="black")


(CSBdata[CSBdata[,"Catch_TXN_Code"]==""])
tmp1<-CSBdata[which(CSBdata[,"Catch_TXN_Code"] %in% CatchTaxa),"TFN"]
tmp2<-unique(CSBdata[CSBdata[,"TFN"]==6 | CSBdata[,"TFN"]==5,c("GAR_Code","GTY_Code","GTY_rev","Catch_TXN_Code","Split_Year")])
 print(tmp2[order(tmp2[,"GAR_Code"],tmp2[,"GTY_rev"],tmp2[,"Catch_TXN_Code"],tmp2[,"Split_Year"]),])      
```  
