...
Depth_intervals<-data.frame("DepthRange" = c("Groundfish","Icefish","Toothfish","Silverfish","Myctophids","Krill","Squid","Crabs")
                           ,"Minimum"    = c(-100,-100, -600,-100,  -300,  -600,  -600,-100)
                           ,"Maximum"    = c(-600,-700,-1800,-700,-10000,-10000,-10000,-800)
                            )# end data frame
...
dfR <- Cbed_rules[
          (Cbed_rules[,"ASD"]=="48.1" & 
                  ( (Cbed_rules[,"SSMU"]=="APBSE" & Cbed_rules[,"Lon"]>-56 & Cbed_rules[,"Lon"]>-63.3) |
                    (Cbed_rules[,"SSMU"]=="APE"   & Cbed_rules[,"Lon"]>-56 & Cbed_rules[,"Lon"]>-63.3) ) | 
                  Cbed_rules[,"SSMU"]=="APDPW" | 
                  Cbed_rules[,"SSMU"]=="APDPE" | 
                  Cbed_rules[,"SSMU"]=="APEI"   )
          ,]
...
RuleDescription <- "Subarea 48.1-longline-Chile - likely to be around Elephpant Island."
...
RuleDescription <- "Division 58.4.1-longline-check catch limits and reports one year later"
RuleFactorLevels <- list(GTY_rev = "LLS",GAR_Code = 5841)

dfR <- Cbed_rules[
          (  Cbed_rules[,"ASD"]=="58.4.1" 
 #          & Cbed_rules[,"SSRU"]=="APEI"     # add SSRUs later ########################
           )
          ,]
...
B09   Area 88         - trawl    - POL - check catch is not pelagic.  apply M02
